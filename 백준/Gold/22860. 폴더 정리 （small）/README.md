...
메모리: 31680 KB, 시간: 164 ms

### 분류

자료 구조(data_structures), 깊이 우선 탐색(dfs), 그래프 이론(graphs), 그래프 탐색(graph_traversal), 해시를 사용한 집합과 맵(hash_set), 구현(implementation), 파싱(parsing), 시뮬레이션(simulation), 문자열(string), 트리(trees)
